...
hospital_data_missing = read_csv(file = "./data/Hospital_General_Information.csv", col_names = TRUE) %>% 
  ...
  mutate(coordinates = str_sub(coordinates, 1, -2)) %>% 
  filter(is.na(coordinates)) %>% 
  mutate(coordinates = map(address, ggmap::geocode)) %>% View()

hospital_data_missing <- hospital_data %>% 
  mutate(coordinates = str_sub(coordinates, 1, -2)) %>% 
  filter(is.na(coordinates))

chrystie_address <- chrystie_address %>% 
  mutate(address = mapply(FUN = function(longitude_avg, latitude_avg) revgeocode(c(longitude_avg, latitude_avg)), chrystie_address$longitude_avg, chrystie_address$latitude_avg))
