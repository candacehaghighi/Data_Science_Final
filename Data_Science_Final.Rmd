...
hospital_data_missing_2 <- hospital_data_missing %>% 
  select(provider_id, hospital_name, coordinates) %>% 
  unnest() %>% 
  filter(is.na(lon)) %>% 
  mutate(coordinates_2 = map(hospital_name, ggmap::geocode))

hospital_data_missing_2 <- hospital_data_missing_2 %>% 
  select(everything(), -lon, -lat) %>% 
  unnest() %>% 
  rename(Longitudes = lon, Latitudes = lat)

hospital_data_2 <-
  left_join(hospital_data, hospital_data_missing_2, by = "provider_id") %>% 
  select(everything(), -c(hospital_name.y:Latitudes.y)) %>% 
  rename(Latitudes = Latitudes.x,
         Longitudes = Longitudes.x,
         hospital_name = hospital_name.x)
...
hospital_name <- vhonor_roll_html %>% html_nodes("td:nth-child(2)") %>% html_text() %>% data.frame() %>% rename(hospital = ".")
